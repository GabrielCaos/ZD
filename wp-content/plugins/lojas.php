<?php 
  	...
  	/*
	Plugin Name: Lojas
	Description: Plugin para criar e gerenciar o post type lojas
	Version: 1.0
	...
	*/
  	...
  	add_action('init', 'create_Lojas' );
  	//add_action("admin_init", "criaBoxLojas");
  	//add_action('save_post', 'saveDataLojas');

  	$prefix = "lojas";
	
	function create_Lojas() 
	{
		$labels = array(
				'name' => __( 'Lojas'),
				'singular_name' => __( 'Loja' ),
				'add_new' => __('Nova Loja'),
				'add_new_item' => __('Nova Loja'),
				'edit_item' => __('Editar Loja'),
				'new_item' => __('Nova Loja'),
				'all_items' => __('Todas as Lojas'),
				'view_item' => __('Ver as Lojas Publicadas'),
				'search_items' => __('Procurar nas Lojas'),
				'not_found' =>  __('Nenhuma Loja'),
				'not_found_in_trash' => __('No Lojas found in Trash'),
				'menu_name' => 'Lojas'
			);
	...
	//Adiciona conteúdo dentro da caixa e atribuindo os campos as variáveis
		/*function addConteudoInfoLojas()
		{
			...
			if (get_post_type( $post ) == "lojas") {
				...
				echo'<label>destaque: <input size="39" id="destaque_lojas" name="destaque_lojas" type="checkbox"'; if ($destaque_lojas == "on") echo 'checked="checked"'; echo'" /></label><br /><br />
					
					<label>imagem: <input size="39" id="imagem_lojas" name="imagem_lojas" type="text" value="'; echo $imagem_lojas; echo'"/><input id="upload_image_lojas" value="Upload da Imagem" type="button" /></label><br /><br />
